...
O case original exige consistência eventual para disponibilidade, mas o README ainda não separa visualmente a consistência forte do inventário, a projeção de leitura em cache e os efeitos assíncronos. Os diagramas raster antigos concentram texto e setas demais, dificultam a leitura e já foram substituídos por outras vistas sem serem removidos.

## Goals

- [ ] Explicar onde existe consistência forte e onde existe convergência eventual.
- [ ] Mostrar as peças AWS efetivamente usadas pela demo sem confundir cache ou fila com fonte de verdade.
- [ ] Separar a topologia demo validada da arquitetura high-load planejada.
- [ ] Substituir os diagramas raster antigos por SVGs legíveis e remover assets sem uso.
...
1. WHEN the consistency visual presents reservation creation THEN it SHALL show that PostgreSQL commits inventory decrement, customer, `PENDING` reservation, and both outbox events in one transaction before the API returns `201`.
2. WHEN the visual presents availability reads THEN it SHALL show that only `GET /events/{id}` uses Valkey cache-aside, a miss reads PostgreSQL, and `GET /reservations/{id}` reads PostgreSQL directly.
...
1. WHEN a committed outbox event is pending THEN the worker SHALL be shown polling PostgreSQL and publishing `ReservationExpirationScheduled` or `ReservationCreated` to separate SQS queues.
...
1. WHEN the demo topology is shown THEN it SHALL include API Gateway REST, WAF, VPC Link v2, internal ALB, three ECS Fargate services, RDS PostgreSQL Single-AZ, single-node Valkey, two SQS queues with DLQs, SES, CloudWatch, Secrets Manager, one NAT Gateway, and the status applied, validated, and destroyed.
...
| --- | --- | --- | --- |
| AWSVIS-01 | Entender a fronteira de consistência | Specify | Approved |
| AWSVIS-02 | Entender os efeitos assíncronos | Specify | Approved |
| AWSVIS-03 | Comparar as escolhas AWS | Specify | Approved |
